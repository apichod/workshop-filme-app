...
// Protège une route API réservée à un formateur connecté (ex: /api/formateur/*)
// — un formateur n'a accès qu'à ses propres sessions, jamais à l'admin complet.
export function requireFormateurAuth(handler) {
  return (req, res) => {
    const session = getSession(req);
    if (!session?.isFormateur) return res.status(401).json({ error: 'Non authentifié' });
    req.formateur = session;
    return handler(req, res);
  };
}

// Protège une page formateur (getServerSideProps) — redirige vers /login sinon
export function withFormateurAuth(gssp) {
  return async (ctx) => {
    const session = getSession(ctx.req);
    if (!session?.isFormateur) {
      return { redirect: { destination: '/login', permanent: false } };
    }
    ctx.formateur = session;
    return gssp(ctx);
  };
}

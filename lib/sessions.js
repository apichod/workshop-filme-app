...
import { sendWorkshopCancelledDateTaken, sendSessionCancelledByAdmin } from './mailer';
...
// supabase_sessions_sort_order_migration.sql pas encore exécutée) — même
// filet de sécurité que pour les formations/formateurs (lib/topics.js,
// lib/formateurs.js) : on retombe sur l'ordre par date plutôt que de faire
// planter tout le chargement de la page (getServerSideProps englobe cet
// appel dans un Promise.all avec les formations/formateurs).
async function selectOpenSessionRows(today) {
  let { data, error } = await supabaseAdmin
    .from('workshop_sessions')
    .select('id, topic_id, session_date, validated, sort_order, workshop_registrations(count)')
...
// Renvoie toutes les sessions à venir (aujourd'hui inclus), dans l'ordre
// manuel "En vedette" (sort_order) — les tris "Taux de remplissage" / "Date"
// proposés sur la page ré-ordonnent ensuite ce tableau côté client sans
// affecter cet ordre de base.
export async function getOpenSessions() {
  const today = isoDate(new Date());

  const [data, topics] = await Promise.all([
    selectOpenSessionRows(today),
  ...
  return (data || [])
    .map((row) => {
      ...
      return {
        ...
        // encore pleine. Exception : un seuil à 0 (priorité totale — souvent
        // combiné à une date fixe) est déjà "confirmé" quel que soit le nombre
        // d'inscrits réels ; le classer systématiquement à 100% le figerait en
        // tête du tri "Taux de remplissage" même sans aucun inscrit. On se base
        // alors sur le remplissage réel (count/capacité) pour qu'il participe
        // normalement au tri, comme n'importe quelle autre session.
        rate: threshold === 0 ? (capacity ? count / capacity : 0) : Math.min(count / threshold, 1),
      };
    })
...
// sessions listées (l'appelant envoie systématiquement l'ordre complet).
export async function reorderSessions(orderedIds) {
  await Promise.all(
    orderedIds.map((id, index) =>
      supabaseAdmin.from('workshop_sessions').update({ sort_order: index }).eq('id', id)
    )
  );
}

// Toutes les sessions à venir (avec ou sans inscrit) et le détail de leurs
// inscrits — pour la popup admin "Préférences → Inscriptions" : retrait
// manuel d'un inscrit, et suppression d'une session entière (ex. une date
// restée programmée par erreur — une date fixe retirée d'une formation après
// coup, par exemple — sans inscrit à prévenir ou non).
...
// Sessions à venir des formations assignées à un formateur donné — pour son
// tableau de bord (pages/formateur.js). Un formateur ne voit que ce qui le
// concerne : réutilise la même source que la vue admin, filtrée par formateur.
export async function getSessionsForFormateur(formateurId) {
  const all = await getSessionsWithRegistrantsAdmin();
  return all.filter((s) => (s.topic?.formateurIds || []).includes(formateurId));
}

// Suppression manuelle d'une session entière depuis l'admin (ex. une date
// restée programmée alors que la formation n'y est plus rattachée). Prévient
// les inscrits concernés par email s'il y en a, puis supprime la session — la
// suppression entraîne celle de ses inscriptions (on delete cascade).
export async function deleteSessionById(sessionId) {
  const { data: session, error } = await supabaseAdmin
    .from('workshop_sessions')
    .select('id, topic_id, session_date, workshop_registrations(name, email)')
    .eq('id', sessionId)
    .maybeSingle();
  if (error) throw error;
  if (!session) return { ok: false, error: 'not_found' };

  const registrants = session.workshop_registrations || [];
  if (registrants.length > 0) {
    const topics = await getAllTopics();
    const topic = topics.find((t) => t.id === session.topic_id) || { title: 'Cette formation' };
    const dateLabel = formatSaturday(session.session_date);
    await sendSessionCancelledByAdmin(registrants, topic, dateLabel);
  }

  const { error: delErr } = await supabaseAdmin.from('workshop_sessions').delete().eq('id', sessionId);
...
// Réserve immédiatement la date fixe d'une formation : crée tout de suite sa
// propre session (pour qu'elle apparaisse dans "Formations à venir" avec sa
// barre de progression, même avant toute inscription) et annule tout autre
// évènement déjà programmé ce jour-là (même validé). Appelée dès que l'admin
// enregistre/modifie une date fixe.
export async function enforceFixedDateExclusivity(topic) {
  ...
  const { data: existing, error: fetchErr } = await supabaseAdmin
    ...
    .eq('session_date', topic.fixedDate)
    .maybeSingle();
  if (fetchErr) throw fetchErr;
  if (!existing) {
    // Un seuil de validation à 0 confirme la session dès sa création (0
    // inscrit suffit) ; sinon elle démarre non validée, comme toute session.
    const { error: insertErr } = await supabaseAdmin
      .from('workshop_sessions')
      .insert({ topic_id: topic.id, session_date: topic.fixedDate, validated: effectiveThreshold(topic) === 0 });

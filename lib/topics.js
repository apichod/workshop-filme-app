...
function mapTopic(row) {
  return {
    ...
    // Plusieurs formateurs possibles (cf. workshop_topic_formateurs, table de
    // liaison many-to-many — supabase_topic_formateurs_migration.sql). Repli
    // sur l'ancienne colonne formateur_id unique si l'embed n'est pas
    // disponible (migration pas encore exécutée, cf. fetchTopicRows).
    formateurIds: Array.isArray(row.workshop_topic_formateurs)
      ? row.workshop_topic_formateurs.map((r) => r.formateur_id)
      : (row.formateur_id ? [row.formateur_id] : []),
    formateurId: row.formateur_id || null, // conservé pour compat descendante (1er formateur)
...
const TOPIC_SELECT_WITH_FORMATEURS = '*, workshop_topic_formateurs(formateur_id)';

// Repli si la table de liaison workshop_topic_formateurs n'existe pas encore
// (migration supabase_topic_formateurs_migration.sql pas encore exécutée) —
// même filet de sécurité que pour sort_order (lib/formateurs.js,
// lib/sessions.js) : on retombe sur l'ancienne colonne formateur_id unique
// plutôt que de faire planter tout le chargement de la page.
async function fetchTopicRows(applyFilters) {
  let { data, error } = await applyFilters(
    supabaseAdmin.from('workshop_topics').select(TOPIC_SELECT_WITH_FORMATEURS)
  );
  if (error) {
    ({ data, error } = await applyFilters(supabaseAdmin.from('workshop_topics').select('*')));
  }
...
export async function getVisibleTopics() {
  const data = await fetchTopicRows((q) => q.eq('archived', false).order('sort_order', { ascending: true }));
...
export async function getAllTopics() {
  const data = await fetchTopicRows((q) => q.order('sort_order', { ascending: true }));
  return (data || []).map(mapTopic);
}

export async function getTopicById(id) {
  const data = await fetchTopicRows((q) => q.eq('id', id).maybeSingle());
...
export async function createTopic({ title, level, desc, fullDescription, program, price, duration, category, type, bonus, maxParticipants, equipment, minParticipants, scheduleMode, fixedDate, formateurIds }) {
  ...
  const cleanFormateurIds = Array.isArray(formateurIds) ? [...new Set(formateurIds.filter(Boolean))] : [];
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      formateur_id: cleanFormateurIds[0] || null, // conservé pour compat descendante (1er formateur)
      archived: false,
  ...
  if (cleanFormateurIds.length) {
    // Repli silencieux si la table de liaison n'existe pas encore (migration
    // supabase_topic_formateurs_migration.sql pas encore exécutée) : le 1er
    // formateur reste au moins assigné via l'ancienne colonne formateur_id.
    const { error: linkErr } = await supabaseAdmin
      .from('workshop_topic_formateurs')
      .insert(cleanFormateurIds.map((formateurId) => ({ topic_id: id, formateur_id: formateurId })));
    if (linkErr) console.error('[createTopic] workshop_topic_formateurs insert', linkErr);
  }
...
export async function updateTopic(id, fields) {
  ...
  // Plusieurs formateurs possibles (cf. workshop_topic_formateurs) — champ
  // formateurId (singulier) conservé pour compat descendante (anciens appels,
  // imports JSON) : remplace alors toute l'assignation par ce seul formateur.
  let cleanFormateurIds = null;
  if (fields.formateurIds !== undefined) {
    cleanFormateurIds = Array.isArray(fields.formateurIds) ? [...new Set(fields.formateurIds.filter(Boolean))] : [];
    patch.formateur_id = cleanFormateurIds[0] || null;
  } else if (fields.formateurId !== undefined) {
    cleanFormateurIds = fields.formateurId ? [fields.formateurId] : [];
    patch.formateur_id = fields.formateurId || null;
  }
  ...
  if (cleanFormateurIds !== null) {
    // Remplace toute l'assignation existante par la nouvelle liste reçue.
    const { error: delErr } = await supabaseAdmin.from('workshop_topic_formateurs').delete().eq('topic_id', id);
    if (delErr) {
      console.error('[updateTopic] workshop_topic_formateurs delete', delErr);
    } else if (cleanFormateurIds.length) {
      const { error: insErr } = await supabaseAdmin
        .from('workshop_topic_formateurs')
        .insert(cleanFormateurIds.map((formateurId) => ({ topic_id: id, formateur_id: formateurId })));
      if (insErr) console.error('[updateTopic] workshop_topic_formateurs insert', insErr);
    }
    return getTopicById(id);
  }
...
export async function importTopic(t) {
  ...
  // Plusieurs formateurs possibles à l'import : tableau formateurIds/formateur_ids
  // si présent, sinon repli sur l'ancien champ formateur unique.
  const rawFormateurIds = Array.isArray(t.formateurIds) ? t.formateurIds : (Array.isArray(t.formateur_ids) ? t.formateur_ids : null);
  const cleanFormateurIds = rawFormateurIds
    ? [...new Set(rawFormateurIds.filter(Boolean))]
    : (t.formateurId || t.formateur_id ? [t.formateurId || t.formateur_id] : []);
  patch.formateur_id = cleanFormateurIds[0] || null; // conservé pour compat descendante (1er formateur)

  async function syncFormateurLinks() {
    const { error: delErr } = await supabaseAdmin.from('workshop_topic_formateurs').delete().eq('topic_id', id);
    if (delErr) { console.error('[importTopic] workshop_topic_formateurs delete', delErr); return; }
    if (cleanFormateurIds.length) {
      const { error: insErr } = await supabaseAdmin
        .from('workshop_topic_formateurs')
        .insert(cleanFormateurIds.map((formateurId) => ({ topic_id: id, formateur_id: formateurId })));
      if (insErr) console.error('[importTopic] workshop_topic_formateurs insert', insErr);
    }
  }
  ...
  if (existing) {
    const { error } = await supabaseAdmin
      ...
      .single();
    if (error) throw error;
    await syncFormateurLinks();
    return { id, action: 'updated', topic: await getTopicById(id) };
  }
  ...
  if (error) throw error;
  await syncFormateurLinks();
  return { id, action: 'created', topic: await getTopicById(id) };
}

// ─── Jours de la semaine à venir (samedi, mais aussi mercredi/vendredi) ──────

// `isAvailable(dateIso)` : prédicat de disponibilité (salle de cours + tous
// les formateurs assignés à la formation, cf. lib/availability.js). On
// continue à avancer semaine après semaine jusqu'à obtenir `count`
// occurrences réellement disponibles de ce jour de la semaine.
export function nextWeekdayDates(weekday, count = 8, isAvailable = () => true) {
  const dates = [];
  const d = new Date();
  d.setHours(0, 0, 0, 0);
  const diff = (weekday - d.getDay() + 7) % 7;
  d.setDate(d.getDate() + (diff === 0 ? 7 : diff)); // prochaine occurrence à venir
  // Garde-fou : ne boucle pas indéfiniment si (par erreur) aucune date à venir
  // n'était disponible.
  let safety = count + 260;
  while (dates.length < count && safety-- > 0) {
    const iso = isoDate(d);
    if (isAvailable(iso)) dates.push(iso);
    d.setDate(d.getDate() + 7);
  }
  return dates;
}

// Conservé pour compatibilité, toujours centré sur le samedi quel que soit le
// planning des formations.
export function nextSaturdays(count = 8, isAvailable = () => true) {
  return nextWeekdayDates(6, count, isAvailable);
}

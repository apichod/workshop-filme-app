...
function mapTopic(row) {
  return {
    ...
    fixedDate: row.fixed_date || null, // date fixe (n'importe quel jour) qui remplace la liste de samedis et réserve la date en exclusivité
...
export async function createTopic({ title, level, desc, fullDescription, program, price, duration, category, type, bonus, maxParticipants, equipment, minParticipants, fixedDate }) {
...
export async function updateTopic(id, fields) {
  ...
  if (fields.fixedDate !== undefined) patch.fixed_date = fields.fixedDate || null;
...
export async function importTopic(t) {
  ...
  const patch = {
    ...
    fixed_date: t.fixedDate || t.fixed_date || null,
    archived: !!t.archived,
  };
